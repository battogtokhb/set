...
class SetCardView: UIView {
    ...
    var shapeColor: UIColor = #colorLiteral(red: 1, green: 0.1491314173, blue: 0, alpha: 1) {
        didSet {
            setNeedsDisplay()
            setNeedsLayout()
        }
    ...
    var borderColor: UIColor = #colorLiteral(red: 0.2392156869, green: 0.6745098233, blue: 0.9686274529, alpha: 1) {
        didSet {
            setNeedsDisplay()
            setNeedsLayout()
        }
    ...
    var background: UIColor = #colorLiteral(red: 1, green: 1, blue: 1, alpha: 1) {
        didSet {
            setNeedsDisplay()
            setNeedsLayout()
        }
    ...
    private func putStripes(in path: UIBezierPath, on rect: CGRect) {
        ...
        for offset in 0..<numberOfStripes {
            path.move(to: CGPoint(x: CGFloat(offset) *   stripesSpacingFactor, y: rect.minX))
            path.addLine(to: CGPoint(x: CGFloat(offset) *   stripesSpacingFactor, y: rect.maxY) )
        }
        path.lineWidth = SizeRatio.pathLineWidth
        path.stroke()
